...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Выборка.Следующий() Тогда 
		Если ЭтотОбъект.ВКМ_Специалист <> Ссылка.ВКМ_Специалист Тогда
			ОповеститьОбИзмененияхВДокументе(1,ЭтотОбъект.ВКМ_Специалист);
		КонецЕсли;
        Если ЭтотОбъект.ВКМ_ДатаПроведенияРабот <> Ссылка.ВКМ_ДатаПроведенияРабот Тогда
			ОповеститьОбИзмененияхВДокументе(2,ЭтотОбъект.ВКМ_ДатаПроведенияРабот);
		КонецЕсли;
	    Если ЭтотОбъект.ВКМ_ВремяНачалаРабот <> Ссылка.ВКМ_ВремяНачалаРабот Тогда
			ОповеститьОбИзмененияхВДокументе(3,ЭтотОбъект.ВКМ_ВремяНачалаРабот);
		КонецЕсли; 
		Возврат;
	Иначе
...
Функция ПроверкаТипаДатыДокумента(Отказ)
	...
	Если (ВКМ_Договор.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание И НайденныйДоговор = Ложь) или
	НЕ (ВКМ_Договор.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание) Тогда
		Отказ = истина;
